...
public class TimeBaseVerification {
	...
	public String generateChallenge() {
		...
		String res = this.cipher.unsafeEncryptAES(tmp);
		return res;
	}

	public String generateResponse(String challenge) {
		String raw = this.cipher.unsafeDecryptAES(challenge);
	...
	public boolean verifyResponse(String raw) {
		return this.cipher.unsafeDecryptAES(raw).equals(this.response);
	}
	...
	private String makeChallenge(String code) {
		...
		for (int i = 0; i < 32; i++) {
			builder.append(Integer.toHexString(sha512[i] & 0x000000FF | 0xFFFFFF00).substring(6));
		}
	...
	private String makeResponse(String code) {
		...
		String hex;
		for (int i = 0; i < 32; i++) {
			hbit = sha512[2 * i] + 0x80;
			lbit = sha512[2 * i + 1] + 0x80;
