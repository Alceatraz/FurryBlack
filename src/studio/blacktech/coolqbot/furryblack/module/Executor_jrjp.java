...
public class Executor_jrjp extends FunctionExecutor {
	...
	public void executor(final Workflow flow) {
		this.counter++;
		if (flow.from < 3) {
			FunctionExecutor.priWarn(flow, "今日祭品仅支持群");
			return;
		}
		...
		FunctionExecutor.grpInfo(flow, nickname + " (" + Executor_jrjp.jrjp.get(flow.gpid).getQqId() + ") 被作为祭品献祭掉了，召唤出一个神秘视频 https://www.bilibili.com/video/av" + Executor_jrjp.jrav.get(flow.gpid));
	}
