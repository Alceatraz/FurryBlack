...
/***
 * 使用标准JavaCipher包装的AES-128 CBC分组模式工具类， 包含三种加密模式：标准加密、使用HA-384进行消息验证、使用签名后不初始化的SHA-384进行消息验证。 带消息验证的数据帧为： 00 00 00 00 , 00 00 00 00 - 00 00 00 00 , 00 00 00 00 - XXXX 前8位 原始消息getBytes(UTF-8)后数组的长度 int → hexString → getBytes(UTF-8) 后8位 SHA-384的前8位 之后为原始数据getBytes(UTF-8) 数据帧经过AES加密和Base64编码，成为密文。
 *
