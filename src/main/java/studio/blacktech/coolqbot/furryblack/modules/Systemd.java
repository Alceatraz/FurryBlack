...
public class Systemd extends Module {
	...
	public boolean init() throws Exception {
		...
		for (Group group : entry.getCQ().getGroupList()) NICKNAME_MAP.put(group.getId(), new TreeMap<>());
		...
		BufferedReader readerHelp = new BufferedReader(
			new InputStreamReader(new FileInputStream(FILE_MESSAGE_HELP), StandardCharsets.UTF_8));
		BufferedReader readerInfo = new BufferedReader(
			new InputStreamReader(new FileInputStream(FILE_MESSAGE_INFO), StandardCharsets.UTF_8));
		BufferedReader readerEula = new BufferedReader(
			new InputStreamReader(new FileInputStream(FILE_MESSAGE_EULA), StandardCharsets.UTF_8));
		BufferedReader readerMute = new BufferedReader(
			new InputStreamReader(new FileInputStream(FILE_SILENCE_GROP), StandardCharsets.UTF_8));
		BufferedReader readerNick = new BufferedReader(
			new InputStreamReader(new FileInputStream(FILE_NICKNAME_MAP), StandardCharsets.UTF_8));
		...
		while (entries.hasMoreElements()) {
			...
			if (clazz.isAnnotationPresent(ModuleComponent.class)) logger.full("注册核心模块", entryName);
		...
		LIST_TRIGGER_USER = CONFIG_TRIGGER_USER.equals("none") ? new String[0]
			: CONFIG_TRIGGER_USER.split(",");
		LIST_TRIGGER_DISZ = CONFIG_TRIGGER_DISZ.equals("none") ? new String[0]
			: CONFIG_TRIGGER_DISZ.split(",");
		LIST_TRIGGER_GROP = CONFIG_TRIGGER_GROP.equals("none") ? new String[0]
			: CONFIG_TRIGGER_GROP.split(",");
		...
		LIST_LISENTER_USER = CONFIG_LISENTER_USER.equals("none") ? new String[0]
			: CONFIG_LISENTER_USER.split(",");
		LIST_LISENTER_DISZ = CONFIG_LISENTER_DISZ.equals("none") ? new String[0]
			: CONFIG_LISENTER_DISZ.split(",");
		LIST_LISENTER_GROP = CONFIG_LISENTER_GROP.equals("none") ? new String[0]
			: CONFIG_LISENTER_GROP.split(",");
		...
		LIST_EXECUTOR_USER = CONFIG_EXECUTOR_USER.equals("none") ? new String[0]
			: CONFIG_EXECUTOR_USER.split(",");
		LIST_EXECUTOR_DISZ = CONFIG_EXECUTOR_DISZ.equals("none") ? new String[0]
			: CONFIG_EXECUTOR_DISZ.split(",");
		LIST_EXECUTOR_GROP = CONFIG_EXECUTOR_GROP.equals("none") ? new String[0]
			: CONFIG_EXECUTOR_GROP.split(",");
		...
		for (String name : LIST_SCHEDULER) if (SCHEDULER_INSTANCE.containsKey(name)) {
		...
		} else logger.warn("配置错误", "定时器不存在 " + name);
		...
		for (String name : LIST_TRIGGER_USER) if (TRIGGER_INSTANCE.containsKey(name)) {
		...
		} else logger.warn("配置错误", "私聊触发器不存在 " + name);

		for (String name : LIST_TRIGGER_DISZ) if (TRIGGER_INSTANCE.containsKey(name)) {
		...
		} else logger.warn("配置错误", "组聊触发器不存在 " + name);

		for (String name : LIST_TRIGGER_GROP) if (TRIGGER_INSTANCE.containsKey(name)) {
		...
		} else logger.warn("配置错误", "群聊触发器不存在 " + name);
		...
		for (String name : LIST_LISENTER_USER) if (LISTENER_INSTANCE.containsKey(name)) {
		...
		} else logger.warn("配置错误", "私聊监听器不存在 " + name);

		for (String name : LIST_LISENTER_DISZ) if (LISTENER_INSTANCE.containsKey(name)) {
		...
		} else logger.warn("配置错误", "组聊监听器不存在 " + name);

		for (String name : LIST_LISENTER_GROP) if (LISTENER_INSTANCE.containsKey(name)) {
		...
		} else logger.warn("配置错误", "群聊监听器不存在 " + name);
		...
		for (String name : LIST_EXECUTOR_USER) if (EXECUTOR_INSTANCE.containsKey(name)) {
		...
		} else logger.warn("配置错误", "私聊执行器不存在 " + name);

		for (String name : LIST_EXECUTOR_DISZ) if (EXECUTOR_INSTANCE.containsKey(name)) {
		...
		} else logger.warn("配置错误", "组聊执行器不存在 " + name);

		for (String name : LIST_EXECUTOR_GROP) if (EXECUTOR_INSTANCE.containsKey(name)) {
		...
		} else logger.warn("配置错误", "群聊执行器不存在 " + name);
		...
		MESSAGE_LIST_USER = generateListMessage("私聊", TRIGGER_USER, LISTENER_USER,
			EXECUTOR_USER);
		MESSAGE_LIST_DISZ = generateListMessage("组聊", TRIGGER_DISZ, LISTENER_DISZ,
			EXECUTOR_DISZ);
		MESSAGE_LIST_GROP = generateListMessage("群聊", TRIGGER_GROP, LISTENER_GROP,
			EXECUTOR_GROP);
	...
	public String[] exec(Message message) throws Exception {
		...
		if (module == null) return new String[] {
			"参数错误 --module 为空"
		};
					...
					switch (message.getSegment(2)) {
						...
						case "dump":
							...
							for (Group group : entry.getCQ().getGroupList()) {
								...
								for (Member memebr : entry.getCQ().getGroupMemberList(group.getId())) {
									...
									dumper.append(group.getId() + ":" + memebr.getQQId() + ":"
										+ getGropNick(group.getId(), memebr.getQQId()) + "\n");
									i++;
						...
						case "save":
							...
							for (Group group : entry.getCQ().getGroupList()) {
								...
								for (Member memebr : entry.getCQ().getGroupMemberList(group.getId())) {
									...
									if (isMyself(memebr.getQQId())) continue;
									saver.append(group.getId() + ":" + memebr.getQQId() + ":"
										+ getGropNick(group.getId(), memebr.getQQId()) + "\n");
						...
						case "load":
							...
							BufferedReader reader = new BufferedReader(new InputStreamReader(
								new FileInputStream(FILE_NICKNAME_MAP), StandardCharsets.UTF_8));
	...
	public void groupMemberIncrease(int typeid, int sendtime, long gropid, long operid, long userid) throws Exception {
		...
		if (isMyself(userid)) {
			...
			for (Member memeber : entry.getCQ().getGroupMemberList(gropid)) writer.append(gropid + ":" + memeber.getQQId() + ":"
				+ entry.getCQ().getStrangerInfo(memeber.getQQId()).getNick() + "\n");

		} else writer.append("# Member Increase " + LoggerX.datetime() + "\n" + gropid + ":" + userid + ":"
	 ...
	 */
	public void doUserMessage(int typeid, long userid, MessageUser message, int messageid, int messagefont)
		throws Exception {
		...
		if (message.isCommand()) switch (message.getCommand()) {
			...
			case "help":
				if (message.getSection() == 0) this.sendHelp(userid);
				else this.sendHelp(userid, message.getSegment()[0]);
				break;

			default:
				if (ENABLE_EXECUTOR_USER && EXECUTOR_USER.containsKey(message.getCommand())) EXECUTOR_USER.get(message.getCommand()).executeUserMessage(typeid, userid, message,
		...
		else this.userInfo(userid, "未识别的内容，本BOT没有聊天功能，请使用/help查看帮助。");
	 ...
	 * @throws Exception 你永远不应该执行这个方法
	 */
	public void doDiszMessage(long diszid, long userid, MessageDisz message, int messageid, int messagefont)
		throws Exception {
		...
		if (message.isCommand()) switch (message.getCommand()) {
			...
			default:
				if (ENABLE_EXECUTOR_DISZ && EXECUTOR_DISZ.containsKey(message.getCommand())) EXECUTOR_DISZ.get(message.getCommand()).executeDiszMessage(diszid, userid, message,
					messageid, messagefont);
				else {
	 ...
	 * @throws Exception 你永远不应该执行这个方法
	 */
	public void doGropMessage(long gropid, long userid, MessageGrop message, int messageid, int messagefont)
		throws Exception {
		...
		if (message.isCommand()) switch (message.getCommand()) {
			...
			default:
				if (ENABLE_EXECUTOR_GROP && EXECUTOR_GROP.containsKey(message.getCommand())) EXECUTOR_GROP.get(message.getCommand()).executeGropMessage(gropid, userid, message,
					messageid, messagefont);
				else {
	...
	public String[] generateReport(int mode, Message message, Object... parameters) {
		...
		for (ModuleScheduler instance : SCHEDULER_ENABLED) if (instance.ENABLE()) builder.append(instance.MODULE_PACKAGENAME() + "：" + instance.COUNT() + "\r\n");

		builder.append("触发器：" + TRIGGER_USER.size() + "/" + TRIGGER_DISZ.size() + "/"
			+ TRIGGER_GROP.size() + "个" + "\r\n");
		...
		builder.append("监听器：" + LISTENER_USER.size() + "/" + LISTENER_DISZ.size() + "/"
			+ LISTENER_GROP.size() + "个" + "\r\n");
		...
		builder.append("执行器：" + EXECUTOR_USER.size() + "/" + EXECUTOR_DISZ.size() + "/"
			+ EXECUTOR_GROP.size() + "个" + "\r\n");
	...
	public String[] reportAllModules(int mode, Message message, Object... parameters) {
		...
		builder02.append("触发器：" + TRIGGER_USER.size() + "/" + TRIGGER_DISZ.size() + "/"
			+ TRIGGER_GROP.size() + "个" + "\r\n");
		...
		builder03.append("监听器：" + LISTENER_USER.size() + "/" + LISTENER_DISZ.size() + "/"
			+ LISTENER_GROP.size() + "个" + "\r\n");
		...
		builder04.append("执行器：" + EXECUTOR_USER.size() + "/" + EXECUTOR_DISZ.size() + "/"
			+ EXECUTOR_GROP.size() + "个" + "\r\n");
	...
	public String[] reportSpecifiedModule(int mode, Message message, Object... parameters) {
		...
		if (module.contains(":")) {
			...
			if (name.equals("systemd")) report = generateReport(0, message, null, null); // /admin
			// report
			// --module=systemd:$
			else if (TRIGGER_INSTANCE.containsKey(name)) report = TRIGGER_INSTANCE.get(name).generateReport(submode, message, null, null); // /admin
			// report
			// --module=userdeny:0
			else if (LISTENER_INSTANCE.containsKey(name)) report = LISTENER_INSTANCE.get(name).generateReport(submode, message, null, null); // /admin
			// report
			// --module=shui:20 --group=1234567890
			else if (EXECUTOR_INSTANCE.containsKey(name)) report = EXECUTOR_INSTANCE.get(name).generateReport(submode, message, null, null); // /admin
			// report
			// --module=acon --group=1234567890
			else report[0] = "模块不存在";

		} else report[0] = "module参数错误：" + module;
	...
	private String generateListMessage(String flagname, ArrayList<ModuleTrigger> triggers,
		...
		if (triggers.size() == 0) builder.append("无\r\n");
		else {

			builder.append(triggers.size() + "\r\n");

			for (ModuleTrigger module : triggers) builder.append(module.MODULE_COMMANDNAME() + " > " + module.MODULE_DISPLAYNAME() + "："
		...
		if (listeners.size() == 0) builder.append("无\r\n");
		else {

			builder.append(listeners.size() + "\r\n");

			for (ModuleListener module : listeners) builder.append(module.MODULE_COMMANDNAME() + " > " + module.MODULE_DISPLAYNAME() + "："
				+ module.MODULE_DESCRIPTION() + "\r\n");
		...
		if (executors.size() == 0) builder.append("无\r\n");
		else {
			...
			for (String temp : executors.keySet()) {
				...
				module.genFullHelp();
				builder.append(module.MODULE_COMMANDNAME() + " > " + module.MODULE_DISPLAYNAME() + "："
					+ module.MODULE_DESCRIPTION() + "\r\n");
	...
	public void gropInfo(long gropid, String message) {
		...
		if (MESSAGE_MUTE.contains(gropid)) System.out.println("关闭发言 " + gropid + "：" + message);
		else entry.getCQ().sendGroupMsg(gropid, message);
	...
	public void gropInfo(long gropid, String... message) {
		...
		if (MESSAGE_MUTE.contains(gropid)) System.out.println("关闭发言 " + gropid + "：" + message);
		else for (String temp : message) entry.getCQ().sendGroupMsg(gropid, temp);
	...
	public void gropInfo(long gropid, long userid, String message) {
		...
		if (MESSAGE_MUTE.contains(gropid)) System.out.println("关闭发言 " + gropid + "：" + message);
		else entry.getCQ().sendGroupMsg(gropid, "[CQ:at,qq=" + userid + "] " + message);
	...
	public void sendHelp(long userid, String name) {
		...
		if (EXECUTOR_INSTANCE.containsKey(name)) entry.getCQ().sendPrivateMsg(userid, EXECUTOR_INSTANCE.get(name).MODULE_FULLHELP);
		else entry.getCQ().sendPrivateMsg(userid, "不存在名叫" + name + "的模块。请使用/list查询。");
	...
	/**
	 * 获取系统随机生成器 entry对此方法进行了转发请勿在此执行
	 *
	 * @return SecureRandom
	 */
	...
	public int getNextInteger() {
		...
		int temp = random ^ random >>> 20 ^ random >>> 12;
		return temp ^ temp >>> 7 ^ temp >>> 4;
